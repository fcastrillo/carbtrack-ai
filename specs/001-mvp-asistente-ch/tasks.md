...
- [x] T020 [US2] Implement Edge Function `supabase/functions/analyze-meal/index.ts`: read OPENAI_API_KEY from env; receive image (url or base64); call GPT-4o Vision; query Supabase `master_food_list` and match/cross foods; return { items, total_carbs } per contracts/edge-functions.md
